...
const MANAGER_ONLY = new Set(['/agency', '/admin', '/users']);
...
async function onRoute(route) {
  ...
  // Full customer profile: #/customers/<leadId>
  if (
    pathOnly.startsWith('/customers/') &&
    pathOnly !== '/customers/new' &&
    pathOnly !== '/customers/import'
  ) {
    const leadId = decodeURIComponent(pathOnly.slice('/customers/'.length));
    root.innerHTML = '';
    await renderCustomerProfile(root, leadId);
    return;
  }

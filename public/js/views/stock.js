...
export async function renderStock(root) {
  const canCreate = can('canCreateVehicle');
  const canImport = can('canImportVehicles');
  root.innerHTML = `
    <div class="page-head">
      <div>
        <h1>מלאי רכבים</h1>
      ...
      <div class="actions-row" style="margin:0">
        ${canCreate ? `<a class="btn btn-primary" href="#/stock/new">הוסף רכב חדש</a>` : ''}
        ${canImport ? `<label class="btn btn-secondary file-label">ייבוא מלאי (אקסל)
      ...
      { key: 'daysInStock', label: 'ימים', sortable: true, filterable: true },
      {
        ...
        render: (r) =>
          `<div class="actions-row" style="margin:0;gap:0.25rem;flex-wrap:wrap">
            ...
            ${can('canEditVehicle') ? `<a class="btn btn-small btn-secondary" href="#/stock/edit?id=${encodeURIComponent(r.id)}">עריכה מלאה</a>` : ''}
          </div>`,
      },
    ],
    onChange: () => load(),
